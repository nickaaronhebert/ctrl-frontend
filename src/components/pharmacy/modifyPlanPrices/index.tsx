    ...
    if (allMedications?.data) {
      ...
      allMedications.data.forEach((medication: PharmacyCatalogue) => {
        medication.productVariant.forEach((variant: PharmacyProductVariant) => {
          if (!(variant?.pharmacyCatalogue?._id! in prices)) {
            initialPrices[variant?.pharmacyCatalogue?._id!] =
              variant?.newPrice!.toString();
          }
          if (variant?.pharmacyIdentifier) {
            initialIdentifiers[variant?.pharmacyCatalogue?._id!] =
  ...
  return (
    <div className="mb-5">
      <div className="bg-lilac py-3 px-12 flex justify-between mb-4">
        <div>
          <Link
            to={`/pharmacy/medications/plan-catalogues/${id}`}
